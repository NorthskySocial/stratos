...
test.describe('Private Post with Image', () => {
  test.beforeEach(async ({ page }) => {
    ...
    await page.route('**/xrpc/zone.stratos.attestation.verify**', async (route) => {
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
  ...
  test('should create a private post with an image', async ({ page }) => {
    ...
    await page.route(url => url.href.includes('uploadBlob'), async (route) => {
      blobUploaded = true
      await page.evaluate(() => { (window as unknown as { blobUploaded: boolean }).blobUploaded = true })
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        headers: { 'Access-Control-Allow-Origin': '*' },
    ...
    await page.route(url => url.href.includes('createRecord'), async (route) => {
      ...
      expect(body.record.embed.images[0].image.ref.$link).toBe('bafkrei-mock-cid')
      
      recordCreated = true
      await page.evaluate(() => { (window as unknown as { recordCreated: boolean }).recordCreated = true })
      await route.fulfill({
        status: 200,
        contentType: 'application/json',
        headers: { 'Access-Control-Allow-Origin': '*' },
    ...
    try {
      await page.waitForFunction(() => (window as unknown as { blobUploaded?: boolean, recordCreated?: boolean }).blobUploaded && (window as unknown as { blobUploaded?: boolean, recordCreated?: boolean }).recordCreated, null, { timeout: 10000 })
    } catch (e) {
      const errorText = await page.locator('.error').innerText().catch(() => 'No error in UI')
