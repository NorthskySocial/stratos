...
describe('Feedgen room client', () => {
  it('uses only getFeed through the PDS proxy and preserves a feed cursor', async () => {
    ...
    const page = await getFeed(
      current as never,
      { feedgenDid: 'did:web:feed.example' },
      {
        feed: 'terminal-dogma',
        limit: 200,
        cursor: 'asuka-cursor',
    ...
    expect(current.fetchHandler).toHaveBeenCalledWith(
      ...
      {
        method: 'GET',
        headers: { 'atproto-proxy': 'did:web:feed.example#stratos_feedgen' },
      },
    )
  })
  ...
  it('surfaces the Feedgen domain error instead of an empty feed', async () => {
    const current = session(
      new Response(JSON.stringify({ error: 'BoundaryMismatch' }), {
        status: 403,
      }),
    )
    await expect(
      getFeed(
        current as never,
        { feedgenDid: 'did:web:feed.example' },
        { feed: 'tokyo-3', limit: 50 },
      ),
    ).rejects.toMatchObject({ code: 'BoundaryMismatch' })
  })

  it('treats only text fields as post text', () => {
    expect(safePostText({ text: 'Shinji <b>stays text</b>' })).toBe(
      'Shinji <b>stays text</b>',
    )
