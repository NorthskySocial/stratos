...
const ROOM_VISUAL_OPTIONS: readonly RoomVisualOption[] = [
  { icon: 'music', tone: 'magenta', keywords: ['music', 'dance', 'ball', 'club', 'sound'] },
  { icon: 'camera', tone: 'sky', keywords: ['art', 'camera', 'film', 'media', 'photo', 'visual'] },
  { icon: 'comment', tone: 'amber', keywords: ['chat', 'conversation', 'discuss', 'talk'] },
  { icon: 'heart', tone: 'coral', keywords: ['care', 'health', 'heart', 'support', 'wellbeing'] },
  { icon: 'star', tone: 'yellow', keywords: ['culture', 'featured', 'night', 'show'] },
  { icon: 'bookmark', tone: 'orchid', keywords: ['book', 'learn', 'read', 'study', 'writing'] },
  { icon: 'lightning', tone: 'tangerine', keywords: ['action', 'build', 'game', 'project', 'tech'] },
  { icon: 'shield', tone: 'cyan', keywords: ['private', 'safety', 'secure', 'trusted'] },
  { icon: 'gift', tone: 'violet', keywords: ['celebrate', 'party', 'surprise'] },
  { icon: 'home', tone: 'leaf', keywords: ['community', 'house', 'local', 'neighbour'] },
  { icon: 'clock', tone: 'lumen', keywords: ['event', 'meet', 'schedule', 'time'] },
  { icon: 'category', tone: 'mint', keywords: ['collective', 'general', 'group', 'room'] },
]
...
function preferredOption(room: RoomCatalogEntry): RoomVisualOption | undefined {
  const searchable = `${room.id} ${room.displayName} ${room.description}`.toLowerCase()
...
export function roomVisualsFor(
  ...
  for (const room of orderedRooms) {
    ...
    if (option) visuals.set(room.id, { icon: option.icon, tone: option.tone })
  }
