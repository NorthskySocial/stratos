...
describe('extractBoundaries', () => {
  it('extracts boundary values from a record', () => {
    const record = {
      ...
      boundary: {
        values: [{ value: 'did:web:nerv.tokyo.jp/engineering' }, { value: 'did:web:nerv.tokyo.jp/leadership' }],
      },
    }

    expect(extractBoundaries(record)).toEqual(['did:web:nerv.tokyo.jp/engineering', 'did:web:nerv.tokyo.jp/leadership'])
  })
  ...
  it('filters out non-string values', () => {
    const record = {
      boundary: {
        values: [{ value: 'did:web:nerv.tokyo.jp/valid' }, { value: 42 }, {}, { value: null }],
      },
    }
    expect(extractBoundaries(record as never)).toEqual(['did:web:nerv.tokyo.jp/valid'])
  })
})
