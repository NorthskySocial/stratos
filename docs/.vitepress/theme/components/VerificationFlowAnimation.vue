  ...
  <div class="anim-outer" ref="containerRef">
    <div class="stage" ref="stageRef">
      ...
      <svg class="arsvg" viewBox="0 0 900 520">
        <defs>
          <marker id="vf-ml" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto">
            <polygon points="0 0,7 2.5,0 5" fill="#9145EC"/>
          </marker>
          <marker id="vf-mg" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto">
            <polygon points="0 0,7 2.5,0 5" fill="#24cf6e"/>
          </marker>
        </defs>
        <!-- AppView → PDS (diagonal up-right) -->
        <path id="vf-a1" class="ar" stroke="#9145EC" marker-end="url(#vf-ml)" d="M 190 222 L 600 160"/>
        <!-- AppView → DID Resolver (diagonal down-right) -->
        <path id="vf-a2" class="ar" stroke="#7780DC" marker-end="url(#vf-ml)" d="M 190 240 L 600 342"/>
        <!-- AppView → Records (vertical down) -->
        <path id="vf-a3" class="ar" stroke="#24cf6e" marker-end="url(#vf-mg)" d="M 107 278 L 107 375"/>
      </svg>

      <!-- Left: AppView (verifier) -->
      <div class="node" id="vf-nav" style="left:25px;top:175px;width:165px">
        <div class="ni">📡</div>
      ...
      <!-- Top-right: PDS -->
      <div class="node" id="vf-npds" style="left:600px;top:100px;width:155px">
        <div class="ni">🗄️</div>
      ...
      <div class="node" id="vf-ndid" style="left:600px;top:295px;width:175px">
        <div class="ni">🔍</div>
      ...
      <div class="node" id="vf-nrc" style="left:25px;top:375px;width:165px">
        <div class="ni">📄</div>
...
function reset() {
  ;['vf-nav','vf-npds','vf-ndid','vf-nrc'].forEach(id => rm($(id), 'hl', 'ok'))
  ;['vf-a1','vf-a2','vf-a3'].forEach(id => rm($(id), 'show'))
}

const steps = [
  { dur: 1600, fn() { add($('vf-nav'), 'hl') } },
  { dur: 1800, fn() { add($('vf-npds'), 'hl'); add($('vf-a1'), 'show') } },
  { dur: 1800, fn() { rm($('vf-npds'), 'hl'); add($('vf-ndid'), 'hl'); add($('vf-a2'), 'show') } },
  { dur: 2200, fn() { rm($('vf-ndid'), 'hl'); rm($('vf-nav'), 'hl'); add($('vf-nrc'), 'ok'); add($('vf-a3'), 'show') } },
]
...
@keyframes vf-march { to { stroke-dashoffset: -12; } }
.hl { border-color: var(--blu) !important; animation: vf-breathe 2s ease-in-out infinite; }
...
.ar.show { opacity: 1; animation: vf-march .5s linear infinite; }
</style>
