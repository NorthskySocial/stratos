  ...
  <div class="anim-outer" ref="containerRef">
    <div class="stage" ref="stageRef">
      ...
      <svg class="arsvg" viewBox="0 0 900 520">
        <defs>
          <marker id="so-ml" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto">
            <polygon points="0 0,7 2.5,0 5" fill="#9145EC"/>
          </marker>
          <marker id="so-mg" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto">
            <polygon points="0 0,7 2.5,0 5" fill="#24cf6e"/>
          </marker>
        </defs>
        <!-- PDS ↔ Stratos -->
        <path id="so-a1" class="ar" stroke="#9145EC" marker-end="url(#so-ml)" d="M 170 248 L 325 248"/>
        <!-- Stratos → AppView -->
        <path id="so-a2" class="ar" stroke="#9145EC" marker-end="url(#so-ml)" d="M 530 248 L 690 248"/>
        <!-- PDS → DID Resolver (upward) -->
        <path id="so-a3" class="ar" stroke="#7780DC" marker-end="url(#so-ml)" d="M 92 195 L 97 133"/>
        <!-- Stratos → Blob Storage -->
        <path id="so-a4" class="ar" stroke="#9145EC" marker-end="url(#so-ml)" d="M 430 298 L 355 375"/>
        <!-- AppView → PostgreSQL -->
        <path id="so-a5" class="ar" stroke="#24cf6e" marker-end="url(#so-mg)" d="M 755 298 L 640 375"/>
      </svg>

      <!-- Center row -->
      <div class="node" id="so-npds" style="left:15px;top:195px;width:155px">
        <div class="ni">🗄️</div>
        <div class="nn">User PDS</div>
        <div class="ns">pds.bsky.social</div>
      </div>

      <div class="node" id="so-nst" style="left:325px;top:195px;width:205px">
        <div class="ni">⚙️</div>
        <div class="nn">Stratos Service</div>
        <div class="ns">API · OAuth · repo</div>
      </div>

      <div class="node" id="so-nav" style="left:690px;top:195px;width:165px">
        <div class="ni">📡</div>
      ...
      <div class="node" id="so-ndid" style="left:15px;top:30px;width:165px">
        <div class="ni">🔍</div>
      ...
      <!-- Bottom row -->
      <div class="node" id="so-nblob" style="left:240px;top:375px;width:175px">
        <div class="ni">📦</div>
        <div class="nn">Blob Storage</div>
        <div class="ns">disk or S3</div>
      </div>

      <div class="node" id="so-ndb" style="left:525px;top:375px;width:175px">
        <div class="ni">🛢️</div>
...
function reset() {
  ;['so-npds','so-nst','so-nav','so-ndid','so-nblob','so-ndb'].forEach(id => rm($(id), 'hl', 'ok'))
  ;['so-a1','so-a2','so-a3','so-a4','so-a5'].forEach(id => rm($(id), 'show'))
}

const steps = [
  { dur: 1500, fn() { add($('so-nst'), 'hl') } },
  { dur: 1800, fn() { add($('so-npds'), 'hl'); add($('so-a1'), 'show') } },
  { dur: 1800, fn() { rm($('so-npds'), 'hl'); add($('so-ndid'), 'hl'); add($('so-a3'), 'show') } },
  { dur: 1800, fn() { rm($('so-ndid'), 'hl'); rm($('so-nst'), 'hl'); add($('so-nblob'), 'hl'); add($('so-a4'), 'show') } },
  { dur: 1800, fn() { rm($('so-nblob'), 'hl'); add($('so-nav'), 'hl'); add($('so-a2'), 'show') } },
  { dur: 2000, fn() { rm($('so-nav'), 'hl'); add($('so-ndb'), 'ok'); add($('so-a5'), 'show') } },
]
...
@keyframes so-march { to { stroke-dashoffset: -12; } }
.hl { border-color: var(--blu) !important; animation: so-breathe 2s ease-in-out infinite; }
...
.ar.show { opacity: 1; animation: so-march .5s linear infinite; }
</style>
