  ...
  <div class="anim-outer" ref="containerRef">
    <div class="stage" ref="stageRef">
      ...
      <svg class="arsvg" viewBox="0 0 900 520">
        <defs>
          <marker id="tc-ml" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto">
            <polygon points="0 0,7 2.5,0 5" fill="#9145EC"/>
          </marker>
          <marker id="tc-mg" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto">
            <polygon points="0 0,7 2.5,0 5" fill="#24cf6e"/>
          </marker>
        </defs>
        <!-- Row 1: left → right -->
        <path id="tc-a1" class="ar" stroke="#9145EC" marker-end="url(#tc-ml)" d="M 170 86 L 315 86"/>
        <path id="tc-a2" class="ar" stroke="#9145EC" marker-end="url(#tc-ml)" d="M 515 86 L 660 86"/>
        <!-- Connector: PDS Record down to AppView -->
        <path id="tc-a3" class="ar" stroke="#7780DC" marker-end="url(#tc-ml)" d="M 747 138 L 742 340"/>
        <!-- Row 2: right → left -->
        <path id="tc-a4" class="ar" stroke="#9145EC" marker-end="url(#tc-ml)" d="M 660 391 L 510 391"/>
        <path id="tc-a5" class="ar" stroke="#24cf6e" marker-end="url(#tc-mg)" d="M 340 391 L 180 391"/>
      </svg>
      ...
      <!-- Row 2 (right → left) -->
      <div class="node" id="tc-na" style="left:660px;top:340px;width:165px">
        <div class="ni">📡</div>
        <div class="nn">AppView</div>
        <div class="ns">any verifier</div>
      </div>

      <div class="node" id="tc-nuk" style="left:340px;top:340px;width:170px">
        <div class="ni">🔐</div>
        <div class="nn">Signing Key</div>
        <div class="ns">P-256 per actor</div>
      </div>

      <div class="node" id="tc-nrc" style="left:15px;top:340px;width:165px">
        <div class="ni">📄</div>
...
function reset() {
  ;['tc-nk','tc-ne','tc-nr','tc-na','tc-nuk','tc-nrc'].forEach(id => rm($(id), 'hl', 'ok'))
  ;['tc-a1','tc-a2','tc-a3','tc-a4','tc-a5'].forEach(id => rm($(id), 'show'))
}

const steps = [
  { dur: 1600, fn() { add($('tc-nk'), 'hl') } },
  { dur: 1800, fn() { rm($('tc-nk'), 'hl'); add($('tc-ne'), 'hl'); add($('tc-a1'), 'show') } },
  { dur: 1800, fn() { rm($('tc-ne'), 'hl'); add($('tc-nr'), 'ok'); add($('tc-a2'), 'show') } },
  { dur: 2000, fn() { add($('tc-na'), 'hl'); add($('tc-a3'), 'show') } },
  { dur: 1800, fn() { rm($('tc-na'), 'hl'); add($('tc-nuk'), 'hl'); add($('tc-a4'), 'show') } },
  { dur: 2200, fn() { rm($('tc-nuk'), 'hl'); add($('tc-nrc'), 'ok'); add($('tc-a5'), 'show') } },
]
...
@keyframes tc-march { to { stroke-dashoffset: -12; } }

.hl { border-color: var(--blu) !important; animation: tc-breathe 2s ease-in-out infinite; }
...
.ar.show { opacity: 1; animation: tc-march .5s linear infinite; }
</style>
