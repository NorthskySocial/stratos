  ...
  <div class="anim-outer" ref="containerRef">
    <div class="stage" ref="stageRef">
      ...
      <svg class="arsvg" viewBox="0 0 900 520">
        <defs>
          <marker id="bc-ml" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto">
            <polygon points="0 0,7 2.5,0 5" fill="#9145EC"/>
          </marker>
          <marker id="bc-mg" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto">
            <polygon points="0 0,7 2.5,0 5" fill="#24cf6e"/>
          </marker>
        </defs>
        <!-- Top row: Operator → Stratos → DB -->
        <path id="bc-a1" class="ar" stroke="#9145EC" marker-end="url(#bc-ml)" d="M 170 133 L 305 133"/>
        <path id="bc-a2" class="ar" stroke="#9145EC" marker-end="url(#bc-ml)" d="M 505 133 L 685 133"/>
        <!-- DB → PDS (vertical) -->
        <path id="bc-a3" class="ar" stroke="#7780DC" marker-end="url(#bc-ml)" d="M 762 183 L 762 320"/>
        <!-- PDS → AppView (← left) -->
        <path id="bc-a4" class="ar" stroke="#24cf6e" marker-end="url(#bc-mg)" d="M 685 373 L 470 373"/>
      </svg>
      ...
      <div class="node" id="bc-nst" style="left:305px;top:80px;width:200px">
        <div class="ni">⚙️</div>
        <div class="nn">Stratos Service</div>
        <div class="ns">update · re-sign</div>
      </div>

      <div class="node" id="bc-ndb" style="left:685px;top:80px;width:155px">
        <div class="ni">🛢️</div>
      ...
      <!-- Bottom row -->
      <div class="node" id="bc-npds" style="left:685px;top:320px;width:155px">
        <div class="ni">🗄️</div>
        <div class="nn">User PDS</div>
        <div class="ns">enrollment record</div>
      </div>

      <div class="node" id="bc-nav" style="left:305px;top:320px;width:165px">
        <div class="ni">📡</div>
...
function reset() {
  ;['bc-nop','bc-nst','bc-ndb','bc-npds','bc-nav'].forEach(id => rm($(id), 'hl', 'ok'))
  ;['bc-a1','bc-a2','bc-a3','bc-a4'].forEach(id => rm($(id), 'show'))
}

const steps = [
  { dur: 1600, fn() { add($('bc-nop'), 'hl') } },
  { dur: 1800, fn() { rm($('bc-nop'), 'hl'); add($('bc-nst'), 'hl'); add($('bc-a1'), 'show') } },
  { dur: 1800, fn() { rm($('bc-nst'), 'hl'); add($('bc-ndb'), 'hl'); add($('bc-a2'), 'show') } },
  { dur: 1800, fn() { rm($('bc-ndb'), 'hl'); add($('bc-npds'), 'ok'); add($('bc-a3'), 'show') } },
  { dur: 2200, fn() { add($('bc-nav'), 'ok'); add($('bc-a4'), 'show') } },
]
...
@keyframes bc-march { to { stroke-dashoffset: -12; } }
.hl { border-color: var(--blu) !important; animation: bc-breathe 2s ease-in-out infinite; }
...
.ar.show { opacity: 1; animation: bc-march .5s linear infinite; }
</style>
