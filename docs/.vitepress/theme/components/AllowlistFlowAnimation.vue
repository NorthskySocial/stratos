  ...
  <div class="anim-outer" ref="containerRef">
    <div class="stage" ref="stageRef">
      ...
      <svg class="arsvg" viewBox="0 0 900 520">
        <defs>
          <marker id="al-ml" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto">
            <polygon points="0 0,7 2.5,0 5" fill="#9145EC"/>
          </marker>
          <marker id="al-mg" markerWidth="7" markerHeight="5" refX="7" refY="2.5" orient="auto">
            <polygon points="0 0,7 2.5,0 5" fill="#24cf6e"/>
          </marker>
        </defs>
        <!-- Client → Stratos -->
        <path id="al-a1" class="ar" stroke="#9145EC" marker-end="url(#al-ml)" d="M 170 248 L 310 248"/>
        <!-- Stratos → Eligibility -->
        <path id="al-a2" class="ar" stroke="#9145EC" marker-end="url(#al-ml)" d="M 510 248 L 655 248"/>
        <!-- Stratos → PDS (diagonal down) -->
        <path id="al-a3" class="ar" stroke="#24cf6e" marker-end="url(#al-mg)" d="M 407 298 L 387 375"/>
      </svg>
      ...
      <!-- Center: Stratos -->
      <div class="node" id="al-nst" style="left:310px;top:195px;width:200px">
        <div class="ni">⚙️</div>
      ...
      <div class="node" id="al-nel" style="left:655px;top:195px;width:190px">
        <div class="ni">✅</div>
      ...
      <!-- Bottom: PDS -->
      <div class="node" id="al-npds" style="left:310px;top:375px;width:165px">
        <div class="ni">🗄️</div>
...
function reset() {
  ;['al-nc','al-nst','al-nel','al-npds'].forEach(id => rm($(id), 'hl', 'ok'))
  ;['al-a1','al-a2','al-a3'].forEach(id => rm($(id), 'show'))
}

const steps = [
  { dur: 1600, fn() { add($('al-nc'), 'hl') } },
  { dur: 1800, fn() { rm($('al-nc'), 'hl'); add($('al-nst'), 'hl'); add($('al-a1'), 'show') } },
  { dur: 2000, fn() { rm($('al-nst'), 'hl'); add($('al-nel'), 'hl'); add($('al-a2'), 'show') } },
  { dur: 1800, fn() { rm($('al-nel'), 'hl'); add($('al-nel'), 'ok'); add($('al-a3'), 'show'); add($('al-npds'), 'ok') } },
  { dur: 2000, fn() { add($('al-nc'), 'ok') } },
]
...
@keyframes al-march { to { stroke-dashoffset: -12; } }
.hl { border-color: var(--blu) !important; animation: al-breathe 2s ease-in-out infinite; }
...
.ar.show { opacity: 1; animation: al-march .5s linear infinite; }
</style>
