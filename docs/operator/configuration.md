...
| `STRATOS_ALLOWED_REDIRECT_ORIGINS`     |          | — (none)                | Comma-separated origins the enrollment flow may redirect back to (e.g. `https://app.example`). Empty means no external redirect: `/oauth/callback` returns JSON instead. Loopback is allowed automatically in dev mode. |
