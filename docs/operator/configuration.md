...
| `STRATOS_ALLOWED_REDIRECT_ORIGINS`     |          | — (none)                | Comma-separated origins the enrollment flow may redirect back to (e.g. `https://app.example`). Only needed for a client that publishes no client metadata document; a client that publishes one proves its own target. Loopback is allowed automatically in dev mode. |
