...
```
AppView ──▶ Stratos zonestratos.sync.subscribeRecords (WebSocket)
            { did: "<user-did>", cursor: 0 }
         │
         ▼
Stratos streams commit events:
  { $type: "zonestratos.sync.subscribeRecords#commit",
    seq: 1,
    did: "did:plc:abc",
    ops: [{ action: "create", path: "zonestratos.feed.post/tid", record: {...} }]
  }
         │
         ▼
...
| `zonestratos.sync.getRepo`    | Exports the full repository as a CAR file (all blocks, MST nodes, signed commit) |
| `zonestratos.repo.importRepo` | Imports a repository from a CAR file with CID integrity verification             |
...
```json
{
  ...
  "scope": "atproto repo:zonestratos.actor.enrollment repo:zonestratos.feed.post",
...
curl https://stratos.example.com/xrpc/zonestratos.enrollment.status?did=did:plc:abc123
```
...
Configure the maximum CAR file size for `zonestratos.repo.importRepo`:
...
AppViews index Stratos content by subscribing to the `zonestratos.sync.subscribeRecords` WebSocket
...
const headers = await createServiceAuthHeaders({
  ...
  lxm: 'zonestratos.sync.subscribeRecords',
...
async function subscribeToUser(did: string, cursor?: number) {
  ...
  const url = new URL('wss://stratos.example.com/xrpc/zonestratos.sync.subscribeRecords')
  ...
  ws.on('message', (data) => {
    ...
    if (frame.$type === 'zonestratos.sync.subscribeRecords#commit') {
...
class StratosIndexer {
  ...
  private async subscribeToUser(did: string, cursor?: number) {
    ...
    ws.on('message', async (data) => {
      ...
      if (event.$type === 'zonestratos.sync.subscribeRecords#commit') {
...
curl "localhost:3100/xrpc/zonestratos.enrollment.status?did=did:plc:abc"

# Test WebSocket connectivity
wscat -c "ws://localhost:3100/xrpc/zonestratos.sync.subscribeRecords?did=did:plc:abc"
```
