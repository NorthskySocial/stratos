...
await rpc.post('com.atproto.repo.createRecord', {
  input: {
    ...
    record: {
      ...
      boundary: { values: [{ value: 'WestCoastBestCoast' }] },
    },
  },
})
```
...
const verified = await fetchAndVerifyRecord(serviceUrl, did, collection, rkey, {
  serviceSigningKey: signingKey,
})

// Without signature verification (CID integrity only)
const verified2 = await fetchAndVerifyRecord(serviceUrl, did, collection, rkey)
```
