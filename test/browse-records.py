...
DEFAULT_COLLECTION = "app.stratos.feed.post"
...
def discover_stratos_from_pds(did: str, pds_url: str) -> dict | None:
    ...
    params = urlencode({"repo": did, "collection": "app.stratos.actor.enrollment", "rkey": "self"})
...
def cmd_discover(user: str):
    ...
    if not enrollment:
        print(
            f"        {C.INFO}No app.stratos.actor.enrollment record found.{C.R}",
...
def cmd_interactive():
  ...
  Commands:
    ...
    list <user> [coll]    List records for user (default collection: app.stratos.feed.post)
