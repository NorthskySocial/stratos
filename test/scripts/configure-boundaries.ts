...
async function run() {
  ...
  for (const [key, userDef] of Object.entries(TEST_USERS)) {
    ...
    info(`Setting boundaries for ${userDef.name}: [${userDef.boundaries.join(", ")}]`);
    ...
    try {
      ...
      if (expected.size === actualSet.size && [...expected].every((b) => actualSet.has(b))) {
        pass(
          `${userDef.name} boundaries set`,
          `[${actual.join(", ")}]`,
        );
        passed++;
      } else {
